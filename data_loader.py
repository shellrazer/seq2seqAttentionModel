...
def data_generate(paths):
    ...
    for series in ['Question', 'Dialogue', 'Report']:
        train_set[series] = train_set[series].str.replace(
            "[a-zA-Z0-9]|[\s+\-\|\!\/\[\]\{\}_,.$%^*(+\"\')]+|[:：+——()?【】“”！，。？、~@#￥%……&*（）]+|车主说|技师说|语音|你好|您好", '',
            regex=True)
...
def prepare_dataset(paths, max_lens):
    ...
    w2v_model = Word2Vec(LineSentence(merge), size=128, hs=1, workers=4, iter=50, min_count=2, max_final_vocab=30000)
    ...
    w2v_model.train(LineSentence(paths['after_pad'][3]), epochs=100, total_examples=w2v_model.corpus_count)
    w2v_model.build_vocab(LineSentence(paths['after_pad'][1]), update=True)
    w2v_model.train(LineSentence(paths['after_pad'][1]), epochs=100, total_examples=w2v_model.corpus_count)
    w2v_model.save('./word2vec.model')
...
def batch(BATCH_SIZE, input, extended_input, input_pad_mask, input_oov, output, output_pad_mask):


    input_train, input_test, extended_input_train, extended_input_test, input_pad_mask_train, input_pad_mask_test, \
    input_oov_train, input_oov_test, output_train, output_test, output_pad_mask_train, output_pad_mask_test = train_test_split(
        input, extended_input, input_pad_mask, input_oov, output, output_pad_mask, test_size=0.12, random_state=6)

    input_train, input_test, extended_input_train, extended_input_test, input_pad_mask_train, input_pad_mask_test, \
    input_oov_train, input_oov_test, output_train, output_test, output_pad_mask_train, output_pad_mask_test = \
    ...
    tf.convert_to_tensor(input_oov_train), tf.convert_to_tensor(input_oov_test), \
    ...
    dataset_train = tf.data.Dataset.from_tensor_slices(
        (input_train, extended_input_train, input_pad_mask_train, input_oov_train, output_train, output_pad_mask_train))
    dataset_test = tf.data.Dataset.from_tensor_slices(
        (input_test, extended_input_test, input_pad_mask_test, input_oov_test, output_test, output_pad_mask_test))
    ...
    return dataset_train_batch, dataset_test_batch
...
def main():
    ...
    #print(max_lens)
    #max_lens = [243, 242, 31, 252]
    #w2v_model, max_lens = prepare_dataset(paths, max_lens)
    #print(max_lens)
    w2v_model = Word2Vec.load('./word2vec.model')
